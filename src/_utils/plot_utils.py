...
def setup_style(palette='auto'):
    ...
    # ★ 视觉规范统一到 _utils/plot_style.py（字体、字号、配色、线宽、网格、边距）。
    try:
        from _utils import plot_style as _PS
        _PS.apply(9.2)
    except Exception as _exc:  # 独立运行时退回原有默认值
        print('plot_style 未加载：%s' % _exc)
...
def _ensure_adjustText():
    """使用已安装的 adjustText；缺失时由调用方使用内置标签调整。"""
    try:
